...
class OilIndexData(EClient, EWrapper):
    cl_futures_df_dict = {}
    cl_futures_previous_day_df_dict = {}
    ...
    def initialise(self):
        self.cl_futures_df_dict = {}
        self.cl_futures_previous_day_df_dict = {}
    ...
    def historicalData(self, reqId: int, bar: BarData):
        try:
            ...
            if reqId == 70000:
                ...
                ticker_to_indicator_column = pd.MultiIndex.from_product([['CL'], ['Open', 'High', 'Low', 'Close', 'Volume']])
                single_ticker_candle_df = pd.DataFrame(ohlcv_list, columns=ticker_to_indicator_column, index=[dt])
                self.cl_futures_df_dict[dt] = single_ticker_candle_df 
            ...
            if reqId == 71000:
                ...
                ticker_to_indicator_column = pd.MultiIndex.from_product([['CL'], ['Open', 'High', 'Low', 'Close', 'Volume']])
                single_ticker_candle_df = pd.DataFrame(ohlcv_list, columns=ticker_to_indicator_column, index=[dt])
                self.cl_futures_previous_day_df_dict[dt] = single_ticker_candle_df
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        try:
            if reqId == 70000:
                print(f'clientID: {self.clientId}, CL minute candle, start: {start}, end: {end}')
                #logger.log_debug_msg(f'clientID: {self.clientId}, CL minute candle, start: {start}, end: {end}')
                self.minute_data_fetched = True
            if reqId == 71000:
                print(f'clientID: {self.clientId}, CL daily candle, start: {start}, end: {end}') 
                #logger.log_debug_msg(f'clientID: {self.clientId}, CL daily candle, start: {start}, end: {end}')
            ...
            if self.minute_data_fetched and self.daily_data_fetched:
                ...
                print(f'Slice CL minute candle start range: {start_range}')

                cl_minute_df_list = []
                cl_daily_df_list = []

                for dt, cl_minute_df in self.cl_futures_df_dict.items():
                    cl_minute_df_list.append(cl_minute_df)

                concat_cl_minute_df = pd.concat(cl_minute_df_list, axis=0)
                print(f'CL original concat minute candle start datetime: {concat_cl_minute_df.iloc[[0]].index[0]}, end datetime: {concat_cl_minute_df.iloc[[-1]].index[0]}')
                concat_cl_minute_df = concat_cl_minute_df.loc[start_range:, :]
                
                if concat_cl_minute_df is None or concat_cl_minute_df.empty:
                    print(f'Empty CL minute dataframe')
                    self.data_finished.set()
                    return
                print(f'CL sliced concat minute candle start datetime: {concat_cl_minute_df.iloc[[0]].index[0]}, end datetime: {concat_cl_minute_df.iloc[[-1]].index[0]}')

                for dt, cl_daily_df in self.cl_futures_previous_day_df_dict.items():
                    cl_daily_df_list.append(cl_daily_df)

                concat_cl_daily_df = pd.concat(cl_daily_df_list, axis=0)

                complete_cl_minute_df = append_customised_indicator(concat_cl_minute_df)
                complete_cl_daily_df = append_customised_indicator(concat_cl_daily_df)
                analyse_index_pop(complete_cl_minute_df, complete_cl_daily_df, 'CL')
                analyse_index_dip(complete_cl_minute_df, complete_cl_daily_df, 'CL')
                self.initialise()
                print(f'clientID: {self.clientId}, completed CL minute candle start: {complete_cl_minute_df.iloc[[0]].index.to_list()[0]}, end: {complete_cl_minute_df.iloc[[-1]].index.to_list()[0]}')
                print(f'clientID: {self.clientId}, completed CL daily candle range: {complete_cl_daily_df.index.tolist()}')
                print(f'clientID: {self.clientId}, complete CL data analysis')
                #logger.log_debug_msg(f'clientID: {self.clientId}, completed NQ minute candle start: {complete_cl_minute_df.iloc[[0]].index.to_list()[0]}, end: {complete_cl_minute_df.iloc[[-1]].index.to_list()[0]}')
                #logger.log_debug_msg(f'clientID: {self.clientId}, completed NQ daily candle start: {complete_cl_daily_df.index.tolist()}')
                ...
                #                         'display.precision', 3):
                #     logger.log_debug_msg(complete_cl_minute_df)

                # #debug
                # with pd.option_context('display.max_rows', None,
                #                            'display.max_columns', None,
                #                         'display.precision', 3):
                #     logger.log_debug_msg(complete_cl_daily_df)
        except Exception as e:
            print(traceback.format_exc())
            self.error_list.append(e)
            self.data_finished.set()